...
async function processDuoToolProtocol(text, options) {
  if (jsonFamilyEnabled.value) {
    return processJSONFamily(text, {
      ...
      origin: { ...(options.origin || {}), protocol: 'json-family' },
    });
  }
  ...
  for (let iter = 0; iter < MAX_ITER; iter++) {
    ...
    for (const call of parsed.calls) {
      ...
      if (call.tool === 'panel_send') {
        ...
        result = entry?.result || { ok: false, is_error: true, output: 'El orquestador activo no devolvió el resultado.' };
      }
