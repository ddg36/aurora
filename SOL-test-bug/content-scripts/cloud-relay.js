    ...
    return new Promise(resolve => {
      ...
      const programarCierreChatGPT = (reason = 'site_complete') => {
        ...
        settle = setTimeout(() => {
          ...
          const mdFinal = domToMarkdown(nodoTexto(current));
      ...
      if (esChatGPT) {
        siteObs = new MutationObserver(() => {
          ...
          // último lote de texto llegue al DOM. En background sólo la
          // estabilidad textual puede cerrar el turno.
          if (ocultoSinShim()) return;
          const textoOk = lastText && !/^(thinking|reasoning|pensando|razonando)\.?$/i.test(lastText);
          if (vioGenerando && textoOk) programarCierreChatGPT();
        });
